...
// const CryptoDetails = () => {
//   const { coinId } = useParams();  //takes id in url and used as variable
...
//   let stats = null, genericStats = null;
//   if (!isFetching) {
//     stats = [
...
//                 {link.type}
//               </Title>
//               <a href={link.url} target="_blank" rel="true noreferrer">
//                 {link.name}
//               </a>
//             </Row>
//           ))}
...
const timePeriod  = (value) => {
  ...
  return (
    <Col className="coin-detail-container">
      ...
      <Select defaultValue="7d" className="select-timeperiod" placeholder="Select Timeperiod" onChange = {timePeriod} >
        {time.map((date) => <Option key={date}>{date}</Option>)}
      </Select>
      <LineChart coinHistory={coinHistory} currentPrice={millify(cryptoDetails?.price)} coinName={cryptoDetails?.name} />
      <Col className="stats-container">
